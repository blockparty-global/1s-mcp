...
export function registerDocsTools(opts: RegisterDocsToolsOptions): number {
  ...
  instrumentedTool(server, analytics, transport,
    ...
    async () => {
      ...
      parts.push('- **Remember the channel across restarts (`X402_CHANNEL_DIR`)** _(restart; ⚠️ advanced, optional)_ — **DEFAULT: off (kept in memory; fine for almost everyone).** Only set a folder path here if they specifically want a batch channel to survive a server restart. Safe to skip.');
      parts.push('');
