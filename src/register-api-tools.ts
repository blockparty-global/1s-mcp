 ...
 *
 * The `1s_std_*` (The Standard Reserve) rows below register only when the
...
export const TOOL_META: Record<string, { title: string; annotations: ToolAnnotations; service?: string }> = Object.freeze({
  ...
  // DGP-3 (2026-09-22, Robinhood Chain): STATE + DEEP were replaced by 2DEEP.
  // These two rows are forward-compatible placeholders, same pattern as the
  // 1s_std_flow_charters guard below — they take effect only once
  // @one-source/api-mcp publishes ToolDefs for getTwoDeepToken / getMigration
  // (operationIds on the live openapi.yaml) and the dependency here is bumped
  // to that version. Until then `allTools` doesn't include them, so this is a
  // no-op, not a claim that the tools are live yet.
  ...(allTools.some((tool) => tool.name === '1s_ds_token_2deep') ? {
    '1s_ds_token_2deep':     { title: 'Deepstate 2DEEP Token',      annotations: RO, service: 'onesource-deepstate' },
  } : {}),
  ...(allTools.some((tool) => tool.name === '1s_ds_migration') ? {
    '1s_ds_migration':       { title: 'Deepstate DGP-3 Migration',  annotations: RO, service: 'onesource-deepstate' },
  } : {}),
  ...
  ...(allTools.some((tool) => tool.name === '1s_std_flow_charters') ? {
    '1s_std_flow_charters':     { title: 'TSR Charter Holder Flow',       annotations: RO, service: 'onesource-standard' },
  } : {}),
